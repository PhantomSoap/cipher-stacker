...
use crate::{CipherName, CipherType, Message::{self, AddCipher, RemoveCipher}, components::Component};
...
impl Component for CipherStack {
    fn draw(&self,frame : &mut Frame, area : Rect,focus : bool) {
        ...
        let widget = match self.state {
            CipherStackState::EditingSelected => {todo!()},
            ...
            CipherStackState::ShowHistory => {todo!()},
        };
    ...
    fn handle_key_events(&mut self,key : KeyEvent)  -> Option<Message>{
        ...
        if let CipherStackState::EditingSelected = self.state {
            match key.code {
